...
def test_cart_count(go_to_final_page):
    ...
    assert finish.get_current_url()==FinishPageLocators.finish_page_url,"Finish page url doesn't match"
    assert "Checkout: Complete!" in finish.get_finish_title(),f"Finish page title doesn't match:{finish.get_finish_title()}"
    assert finish.get_final_cart_count() == 0,"Cart has not been reset"
